...
create or replace package "AI_RAS" as
  g_sessionid RAW(16);
  PROCEDURE CREATE_SESSION( auth_header varchar2 );
  PROCEDURE DESTROY_SESSION;
end "AI_RAS";
/
create or replace package body "AI_RAS" as

FUNCTION get_username_from_auth_header(
    auth_header IN VARCHAR2
) return VARCHAR2 IS
...
BEGIN
    l_http_req := UTL_HTTP.begin_request(l_url, 'GET', 'HTTP/1.1');
    UTL_HTTP.set_header(l_http_req, 'Authorization', auth_header);
    ...
    return APEX_JSON.get_varchar2(p_path => 'sub');
EXCEPTION
    WHEN OTHERS THEN
        return '';
END get_username_from_auth_header;

PROCEDURE CREATE_SESSION( auth_header varchar2 ) is
  app_user varchar2(1024) := 'CUSTOMER';
...
BEGIN
  ...
  IF SUBSTR(auth_header, 1, 6) = 'Bearer' then
     username := get_username_from_auth_header( auth_header );
     AI_PLSQL.LOG('AI_RAS.CREATE_SESSION', username );
     if username='marc.gueury@oracle.com' then
        app_user := 'EMPLOYEE';
     end if;
