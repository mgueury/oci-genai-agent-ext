create user APEX_APP identified by &1;
grant unlimited tablespace to APEX_APP;
grant resource, connect to APEX_APP;
...
alter table "APEX_APP"."DOCS_CHUNCK" add constraint "DOCS_FK" foreign key ( "DOC_ID" ) references "DOCS" ( "ID" ) on delete cascade;
...
RETURN clob IS 
    ...
    region varchar2(128);
BEGIN 
    select region into region from AI_AGENT_RAG_CONFIG where key='region';
...
BEGIN
    ...
    OPEN v_results FOR
        select l.id 
        as DOCID, lv.chunk_id as CHUNKID, l.file_name as TITLE, 
...
BEGIN
    ...
    OPEN v_results FOR
        select 
            ...
            vector_distance(embed, to_vector(query_vec)) AS SCORE
        from docs_chunck
        order by score 
        fetch first top_k rows only;
/*
